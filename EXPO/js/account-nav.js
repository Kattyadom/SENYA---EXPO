...
function renderAccountNav(){
 ...
 for(const [label,href] of entries){const a=document.createElement('a');a.className='sign-video-card';a.href=href;const videoName=role==='interpreter'||role==='admin'?null:videos[href];if(videoName){const video=document.createElement('video');video.muted=true;video.defaultMuted=true;video.setAttribute('muted','');video.loop=true;video.playsInline=true;video.setAttribute('playsinline','');video.preload='metadata';video.src='videos/'+videoName+'-h264.mp4';a.append(video);}a.setAttribute('aria-label',label);{const text=document.createElement('span');text.textContent=label;if(videoName)text.className='sign-video-label';a.append(text);}panel.querySelector('.sign-panel-content').append(a);if(videoName){const retry=document.createElement('button');retry.type='button';retry.className='sign-video-play';retry.textContent='Play '+label+' video';retry.hidden=true;const video=a.querySelector('video');retry.onclick=()=>video.play().catch(()=>{retry.textContent='Retry '+label+' video';});video.addEventListener('playing',()=>{retry.hidden=true;});video.addEventListener('error',()=>{retry.hidden=false;});a.after(retry);}}
 document.body.append(panel);const close=()=>{panel.hidden=true;panel.classList.remove('open');document.body.classList.remove('sign-menu-open');menuButton.setAttribute('aria-expanded','false');panel.querySelectorAll('video').forEach(v=>v.pause());menuButton.focus();};menuButton.onclick=()=>{panel.hidden=false;panel.classList.add('open');document.body.classList.add('sign-menu-open');menuButton.setAttribute('aria-expanded','true');panel.querySelectorAll('video').forEach(v=>v.play().catch(()=>{const retry=v.parentElement.nextElementSibling;if(retry)retry.hidden=false;}));panel.querySelector('button').focus();};panel.querySelector('button').onclick=close;panel.addEventListener('keydown',e=>{if(e.key==='Escape')close();if(e.key==='Tab'){const items=[...panel.querySelectorAll('button,a')];const first=items[0],last=items[items.length-1];if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus();}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus();}}});
 if(role!=='admin'){
  const menuAccess=document.createElement('button');menuAccess.type='button';menuAccess.className='sign-accessibility-button';menuAccess.textContent='Accessibility';menuAccess.setAttribute('aria-controls','accessibilityPanel');
  menuAccess.onclick=()=>{close();access.click();document.querySelector('#accessibilityPanel button')?.focus();};
  panel.querySelector('.sign-panel-content').prepend(menuAccess);
 }
