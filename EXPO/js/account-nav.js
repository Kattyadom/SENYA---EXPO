...
function renderAccountNav(){
 ...
 for(const [label,href] of entries){const a=document.createElement('a');a.className='sign-video-card';a.href=href;const videoName=role==='interpreter'||role==='admin'?null:videos[href];if(videoName){const video=document.createElement('video');video.muted=true;video.loop=true;video.playsInline=true;video.preload='metadata';video.src='videos/'+videoName+'.mp4';a.append(video);}a.setAttribute('aria-label',label);{const text=document.createElement('span');text.textContent=label;if(videoName)text.className='sign-video-label';a.append(text);}panel.querySelector('.sign-panel-content').append(a);}
