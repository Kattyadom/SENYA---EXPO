...
(()=>{
 ...
 function apply(){
  ...
  for(const el of document.body.querySelectorAll('*')){
   ...
   measurements.push([el,pixels*levels[size],{value:el.style.getPropertyValue('font-size'),priority:el.style.getPropertyPriority('font-size')}]);
  }
 ...
 function set(value){if(!(value in levels))return;size=value;localStorage.setItem('textSize',size);localStorage.removeItem('senyaInterpreterTextSize');refresh();}
 ...
 window.addEventListener('storage',event=>{if(event.key==='textSize'){size=event.newValue in levels?event.newValue:'normal';refresh();}});
 refresh();
})();
