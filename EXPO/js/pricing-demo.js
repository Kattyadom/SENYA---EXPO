(function (root) {
 ...
 function estimate(minutes, seconds) {
  ...
  if(!plan||!Number.isFinite(seconds)||seconds<0)throw Error('Invalid demo duration.');
 ...
 function pack(minutes, details) {
  ...
  const text=`SENYA demo package: ${plan.minutes} min — ${money(plan.cents)} USD. No payment.\n${details}`;
 ...
 function unpack(details) {
  const match=/^SENYA demo package: (5|15|30|60) min — \$[\d.]+ USD\. No payment\.\n/.exec(details||'');
 ...
 document.addEventListener('DOMContentLoaded',()=>{
  ...
  const render=()=>{
   ...
   document.getElementById('demoPackageSummary').textContent=`${plan.name}: ${plan.minutes} minutes for ${money(plan.cents)} USD. Amount charged today: $0.00 (demo).`;
   ...
   summary.textContent=`Example total: ${money(result.total)} · Additional time: ${money(result.extra)} · Interpreter estimate: ${money(result.earnings)}. No money is charged or paid.`;
  };
