document.addEventListener('DOMContentLoaded',async()=>{
 ...
 const link=document.createElement('a');link.className='help-btn';link.textContent='Join session';link.hidden=true;status.after(link);
 ...
 link.hidden=!['accepted','in_progress'].includes(r.status)||(r.scheduled_at&&Date.parse(r.scheduled_at)>Date.now()+600000);
 link.href='videollamada.html?id='+encodeURIComponent(id);cancel.disabled=!['waiting','assigned','accepted'].includes(r.status);
