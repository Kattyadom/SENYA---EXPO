...
`);
for(const f of ['001_citas.sql','002_profiles.sql','003_admin.sql'])await db.exec(fs.readFileSync(require('node:path').join(__dirname,'../supabase',f),'utf8'));
...
await db.query('insert into auth.users values($1,$2)',[admin,JSON.stringify({role:'admin',firstName:'Admin'})]);
assert.equal((await db.query('select role from profiles where id=$1',[admin])).rows[0].role,'user');
await assert.rejects(as(admin,'select public.admin_applications()'));
await assert.rejects(as(ids[2],"update public.profiles set role='admin' where id=auth.uid()"));
console.log('PASS registration metadata and direct edits cannot grant admin');
await db.query("update profiles set role='admin' where id=$1",[admin]);
await db.query("update interpreter_profiles set verification_status='pending' where user_id=$1",[ids[2]]);
await assert.rejects(as(ids[2],"select public.review_interpreter($1,'verified','')",[ids[2]]));
await assert.rejects(as(admin,"select public.review_interpreter($1,'verified','')",[ids[2]]));
console.log('PASS only admin can review; approval requires certificate');
await db.query("insert into storage.objects(name,bucket_id) values($1,'certificates')",[ids[2]+'/certificate.pdf']);
await as(admin,"select public.review_interpreter($1,'verified','Checked certificate')",[ids[2]]);
assert.equal((await db.query('select verification_status from interpreter_profiles where user_id=$1',[ids[2]])).rows[0].verification_status,'verified');
assert.equal((await db.query('select count(*)::int n from interpreter_reviews')).rows[0].n,1);
await assert.rejects(as(admin,"select public.review_interpreter($1,'rejected','Duplicate')",[ids[2]]));
const applications=(await as(admin,'select public.admin_applications() result')).rows[0].result;
assert.ok(applications.find(x=>x.id===ids[2]).certificates.length===1);
console.log('PASS approval audit, certificate list and duplicate decision protection');
await db.query("update interpreter_profiles set verification_status='pending' where user_id=$1",[ids[3]]);
await assert.rejects(as(admin,"select public.review_interpreter($1,'rejected','')",[ids[3]]));
await as(admin,"select public.review_interpreter($1,'rejected','Certificate missing')",[ids[3]]);
assert.equal((await as(ids[0],'select * from interpreter_reviews')).rows.length,0);
console.log('PASS rejection needs reason; audit hidden from normal users');
await db.close();
